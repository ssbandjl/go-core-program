...
func main() {
	...
	if err := conf.Parse("intercas.conf"); err != nil {
		panic(err)
	}
	...
	if caches == nil {
		panic("no caches section")
	}

	// log.Printf("cores:%v", cores)
	// log.Printf("caches:%v", caches)
	log.Printf("caches\n")
	for k, v := range caches.data {
		log.Printf("k:%v, v:%v", k, v)
	}

	log.Printf("cores\n")
	for k, v := range cores.data {
		log.Printf("k:%v, v:%v", k, v)
	}
}
...
func (c *Config) ParseReader(reader io.Reader) error {
	...
	for {
		...
		// log.Printf("row:%v, spiter:%v", row, c.Spliter)
		...
		if idx > 0 {
			...
			log.Printf(fmt.Sprintf("[INFO] no spliter in key: %s at %d", row, line))
			continue
		}
