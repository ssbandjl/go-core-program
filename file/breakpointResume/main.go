...
func parseFileInfoFrom(resp *http.Response) string {
	...
	log.Printf("内容形式:%+v", contentDisposition)
	...
	log.Printf("文件相对路径:%s", resp.Request.URL.Path)
...
func main() {
	...
	downloader := NewFileDownloader(url, "", "", 10) //10个线程
...
func (d *FileDownloader) Run() error {
	...
	log.Printf("文件总大小:%d", fileTotalSize)
	if err != nil {
		return err
	}
	d.fileSize = fileTotalSize

	jobs := make([]filePart, d.totalPart) //默认为10
	eachSize := fileTotalSize / d.totalPart
	log.Printf("每个线程文件大小:%d", eachSize)
...
func (d FileDownloader) downloadPart(c filePart) error {
	...
	c.Data = bs
	d.doneFilePart[c.Index] = c //文件分片数组
	return nil
...
func (d FileDownloader) getNewRequest(method string) (*http.Request, error) {
	...
	r.Header.Set("User-Agent", "mojocn") //https://github.com/mojocn/base64Captcha
	return r, nil
}
